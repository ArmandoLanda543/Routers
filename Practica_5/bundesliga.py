...
team_list = [Team(Id=1, Team_Name='Bayern Munich', Championships=32 , NumberChampionsLeague=6),
            Team(Id=2, Team_Name='Borussia Dortmund', Championships= 8 , NumberChampionsLeague=1)]

####################################################    GET     ######################################################
@router.get("/bundesliga/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/bundesliga", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/bundesliga", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/bundesliga/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
