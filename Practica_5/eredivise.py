...
team_list = [Team(Id=1, Team_Name='Ajax', Championships=34 , NumberChampionsLeague=4),
            Team(Id=2, Team_Name='Feyenoord', Championships= 15 , NumberChampionsLeague=0)]

####################################################    GET     ######################################################
@router.get("/eredivise/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/eredivise", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/eredivise", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/eredivise/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
