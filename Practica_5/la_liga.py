...
team_list = [Team(Id=1, Team_Name='Barcelona', Championships=26 , NumberChampionsLeague=5),
            Team(Id=2, Team_Name='Real Madrid', Championships= 35 , NumberChampionsLeague=14)]

####################################################    GET     ######################################################
@router.get("/laliga/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/laliga", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/laliga", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/laliga/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
