...
class Team(BaseModel): ##ENTIDAD
    ...
    Team_Name:str
    Championships:int
    NumberConcacafChampions:int

team_list = [Team(Id=1, Team_Name='Monterrey', Championships=5 , NumberConcacafChampions=5),
            Team(Id=2, Team_Name='America', Championships= 13 , NumberConcacafChampions=6)]

####################################################    GET     ######################################################
@router.get("/ligamx/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/ligamx", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/ligamx", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/ligamx/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
