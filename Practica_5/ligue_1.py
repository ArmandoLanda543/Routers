...
team_list = [Team(Id=1, Team_Name='PSG', Championships=10 , NumberChampionsLeague= 0),
            Team(Id=2, Team_Name='Lille', Championships= 4 , NumberChampionsLeague= 0)]

####################################################    GET     ######################################################
@router.get("/ligue1/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/ligue1", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/ligue1", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/ligue1/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
