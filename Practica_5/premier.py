...
team_list = [Team(Id=1, Team_Name='Liverpool', Championships=19 , NumberChampionsLeague=6),
            Team(Id=2, Team_Name='Arsenal', Championships= 13 , NumberChampionsLeague=0)]

####################################################    GET     ######################################################
@router.get("/premier/{user_id}", status_code=status.HTTP_202_ACCEPTED) #PATH
...
@router.get("/premier", status_code=status.HTTP_200_OK) #QUERY
async def get_users():
    return team_list

####################################################    POST    ######################################################

@router.post("/premier", status_code=status.HTTP_201_CREATED) #QUERY
...
###########################put############################################
@router.put("/premier/", status_code=status.HTTP_426_UPGRADE_REQUIRED)
async def userclass(user:Team):
